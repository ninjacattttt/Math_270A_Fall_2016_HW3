...
public:
	T k;
  	...
  	T EnergyDensity(const T& F){return (T).5*k*(F-(T)1)*(F-(T)1);}      //PSI
  	void P(T& P, const T& F){P=k*(F-(T)1);}								//PSI'=SIGMA
  	void dPdF(T& dPdF,const T F){dPdF=k;}								//PSI''
};
  ...
  a(a_input),dX(dX_input),cons_model(cons_input){constrained_nodes.resize(0);}
	...
	void AddForce(TVect& force,const TVect& x,T scale=(T)1){
  	...
  	void AddForceDerivative(SymmetricTridiagonal<T>& A,const TVect& x,T scale=(T)1){
  	  	T dPdF;
  	  	cons_model.dPdF(dPdF,F(x,0));
  	  	TMat2 element_stiffness;
  	  	T entry=scale*dPdF/dX;
		A(1,1)-=entry;											//there is no N_0 basis function
		
		for(int e=1;e<N-1;e++){
      	  	T dPdF;
	  	  	cons_model.dPdF(dPdF,F(x,e));
      	  	TMat2 element_stiffness;
      	  	T entry=scale*dPdF/dX;
	  	  	element_stiffness << -entry, entry, entry, -entry;
	 	   	for(int i=0;i<2;i++){
		  	  	for(int j=i;j<2;j++){							//SymmetricTridiagonal only needs j=i to 2
				  	A(e+i,e+j)+=element_stiffness(i,j);}}
    	}
  	}

  	void AddForceDifferential(TVect& result,const TVect& x,const TVect& dx,T scale=(T)1){
		...
		dfdx(0,0)=0;											//there is no N_0 basis function
		dfdx(0,1)=0;
